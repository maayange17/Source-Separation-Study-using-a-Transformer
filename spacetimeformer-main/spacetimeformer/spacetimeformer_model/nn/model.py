...
class Spacetimeformer(nn.Module):
    ...
    def forward(
        ...
        q,w,e = enc_y.size()
        enc_x = (torch.arange(1,w)/ w*2).view( -1,1)
        dec_x = (torch.arange(w+1,2*w) / w*2).view( -1,1)
        enc_x = enc_x.repeat(q,1,1).to("cuda:0")
        dec_x = dec_x.repeat(q,1,1).to("cuda:0")
        dec_y = torch.zeros(q,w,self.d_yt).to("cuda:0")
