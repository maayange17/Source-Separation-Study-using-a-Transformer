...
class Embedding(nn.Module):
    ...
    def temporal_embed(self, y: torch.Tensor, x: torch.Tensor):
        ...
        c,v,b = time_emb.shape
        g,h,j = y.shape
        if v != h:         
            val_time_inp = torch.cat((time_emb, y[:, 1:, :]), dim=-1)
            q,w,e = val_time_inp.size()
            newzeros = torch.zeros(q,w+1,e)
            newzeros[:,1:,:] = val_time_inp[:,:,:]
            val_time_inp = newzeros.to("cuda:0")
        else:
